...
try
{
	...
	*/
	...
	{	
		//getting page url	
		$custom_page = $_GET['custom_page'];
		if (file_exists('data.json'))
		{
			$file = fopen('data.json', 'r');
			$jData =  fread($file, filesize('data.json'));
			$aData = json_decode($jData , 'true');
		}
		...
		$link = $custom_page ."/".$sublink;	


		
		if (isset($aData))
		{
			if ($profile = checkProfileAuth ($aUserProfiles , $aData , $custom_page , $sublink))
			{
			 	if (file_exists($link))		
				{
					$link = $link. '?p='. sha1($profile) . '&language=' .$lan;		
					$oP->Add('<iframe src="'. $link .'" style="width:100%; height:98%; border:none;">		
					<p>Your browser does not support iframes.</p>		
					</iframe>');		
				}		
				else		
				{		
					$oP->add('<h3>Directory <b>'. $link .'</b> not found!</h3>');		
				}	
			}
			else
			{
				$oP->add('<h3>Page ' . $sublink .' is not available</h3>');
			}

		}else{
			foreach ($aUserProfiles as $profile => $value) {
				if ($profile =='Administrator' && $value==true)
				{
					if (file_exists($link))		
					{
						$link = $link. '?p='. sha1($profile) . '&language=' .$lan;		
						$oP->Add('<iframe src="'. $link .'" style="width:100%; height:98%; border:none; overflow:scroll;">		
						<p>Your browser does not support iframes.</p>		
						</iframe>');		
					}		
					else		
				...
				foreach ($req as $key => $value) {		
					...
					{		
						...
						if (preg_match("/^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}\z/", $value)) {		
							$ss  = substr($value, 0, 10);		
							$gg = substr($value, 10, 9);		
						...
						if (preg_match("/^\p{P}([0-9]{4}-[0-9]{2}-[0-9]{2}),([0-9]{4}-[0-9]{2}-[0-9]{2})\p{P}\z/", $value, $dates)) {		
											
		
							$t1=preg_split("/-/",$dates[1]);	
							$t2=preg_split("/-/",$dates[2]);		
								...
								$new_date1 = mds_to_gregorian($y1 , $m1 , $d1);		
								$new_date1[1]=str_pad($new_date1[1], 2, '0', STR_PAD_LEFT);		
								$new_date1[2]=str_pad($new_date1[2], 2, '0', STR_PAD_LEFT);	

								$new_date2 = mds_to_gregorian($y2 , $m2 , $d2);		
								$new_date2[1]=str_pad($new_date2[1], 2, '0', STR_PAD_LEFT);		
								$new_date2[2]=str_pad($new_date2[2], 2, '0', STR_PAD_LEFT);	
		
								$req[$key] = '['. join("-" , $new_date1).','.join("-" , $new_date2).']';		
								$req['altered'] = 1;

								$ff = fopen("test_dates.php","w");
										fwrite($ff,$req[$key]);
										fclose($ff);		
