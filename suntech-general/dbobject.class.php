...
require_once(APPROOT.'/application/PersianCalendar.php');
...
abstract class DBObject implements iDisplay
{
	...
	protected function _Set($sAttCode, $value)
	{
        ...
        */	
		if ($sAttCode == 'time_spent') 
		{
			...
			$assDate = $this->Get('assignment_date');
			$resDate = strtotime($this->Get('resolution_date'));

			if (preg_match("/^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}\z/", $assDate)) {		
				$ss  = substr($assDate, 0, 10);		
				$gg = substr($assDate, 10, 9);		
				...
				$miladiAssDate = strtotime(join("-" , $new_date) . $gg);	

				$value = $resDate - $miladiAssDate;
	...
	public function GetStrict($sAttCode)
	{
		...
		/*
                *   Code written By Saman AmnNegar
                *
                *   Here the date outputs will be converted from Gregorian to Persian Date
                *
                */
                if (is_string($value)) {
                    if (preg_match("/^[0-9]{4}-[0-9]{2}-[0-9]{2}\z/", $value)) {
                        ...
                        $new_date = gregorian_to_mds($y , $m , $d);
                        $new_date[1]=str_pad($new_date[1], 2, '0', STR_PAD_LEFT);
                        $new_date[2]=str_pad($new_date[2], 2, '0', STR_PAD_LEFT);
